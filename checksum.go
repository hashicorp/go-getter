...
import (
	"bufio"
	"bytes"
	"crypto/md5"  // #nosec G501 - MD5 used only for checksum verification, not cryptographic security // nosemgrep: go.lang.security.audit.crypto.md5.use-of-insecure-md5-hash
...
func newChecksumFromType(checksumType, checksumValue, filename string) (*FileChecksum, error) {
	...
	switch c.Type {
	case "md5":
		c.Hash = md5.New() // #nosec G401 - MD5 used only for checksum verification // nosemgrep: go.lang.security.audit.crypto.md5.use-of-insecure-md5-hash
	case "sha1":
		c.Hash = sha1.New() // #nosec G505 - SHA1 used only for checksum verification // nosemgrep: go.lang.security.audit.crypto.sha1.use-of-insecure-sha1
	case "sha256":
		c.Hash = sha256.New()
	case "sha512":
		c.Hash = sha512.New()
